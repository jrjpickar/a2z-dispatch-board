...
// job's phases, crew, work days and dumpster loads, plus a safety checklist
// and sign-off. The dashboard sheet does the same math live; this module is
// the server's authority for the numbers and draws the PDF sent to Make.
...
/* ---------- The four formulas ---------- */
export function calcPhase(phase, sf, usable) {
  const rate = num(phase.rate);
  const personDays = rate > 0 ? sf / rate : 0;
  let crew, days;
  if (phase.plan === 'days') {
    const target = Math.max(1, Math.round(num(phase.days)) || 1);
    // Days fixed (typed in, or stretched on the timeline): book that many days
    // and work the crew backward so the person-days fit inside them.
    crew = personDays > 0 ? Math.ceil(personDays / target) : 0;
    days = personDays > 0 ? target : 0;
  } else {
    crew = Math.max(1, Math.round(num(phase.crew)) || 1);
    days = personDays > 0 ? Math.ceil(personDays / crew) : 0;      // work days, always rounded up
  }
  const debrisYd3 = (sf / 1000) * num(phase.yld);
  const loads = usable > 0 && debrisYd3 > 0 ? Math.ceil(debrisYd3 / usable) : 0;
  return { personDays, crew, days, debrisYd3, loads };
}
...
export function buildSchedule(input) {
  ...
  const rows = (input.phases || []).filter(p => p.on !== false).map(p => {
    ...
    return { name: p.name, scope: p.scope, rate: num(p.rate), yld: num(p.yld), plan: p.plan === 'days' ? 'days' : 'crew', ...c, from, to };
  });
...
export function normalizeScheduleInput(raw) {
  ...
  const phases = (Array.isArray(raw.phases) ? raw.phases : []).slice(0, 20).map(p => ({
    name: text(p && p.name, 80) || 'Untitled phase',
    scope: text(p && p.scope, 240),
    rate: clamp(num(p && p.rate), 0, 1_000_000),
    yld: clamp(num(p && p.yld), 0, 1000),
    plan: p && p.plan === 'days' ? 'days' : 'crew',
  ...
  return {
    ...
    start,
    sf: clamp(num(raw.sf), 0, 10_000_000),
    dumpSize: clamp(num(raw.dumpSize), 0, 1000),
    usable: clamp(num(raw.usable), 0, 1000),
...
export async function projectSchedulePdf(input, options = {}) {
  ...
  /* Job info grid: 3 columns x 3 rows */
  const cellH = 30, colW = W / 3;
  const gridTop = y;
  const cells = [
    { c: 0, r: 0, span: 2, k: 'Job', v: blank ? '' : input.jobName },
    { c: 2, r: 0, span: 1, k: 'Job start date', v: blank ? '' : (start ? longDate(start) : '') },
    { c: 0, r: 1, span: 2, k: 'Site address', v: blank ? '' : input.jobAddr },
    { c: 2, r: 1, span: 1, k: 'Planned finish', v: blank ? '' : (finish ? longDate(finish) : '') },
    { c: 0, r: 2, span: 1, k: 'Building size', v: blank ? '' : `${fmt(input.sf)} SF`, unit: blank ? 'SF' : '' },
    { c: 1, r: 2, span: 1, k: 'Dumpster load size', v: blank ? '' : `${fmt(input.dumpSize)} cu yd roll-off  |  ${fmt(input.usable)} cu yd usable`, unit: blank ? 'cu yd box  /  ____ cu yd usable' : '' },
    { c: 2, r: 2, span: 1, k: 'Schedule mode', v: blank ? '' : modeLabel(input.mode), mode: blank }
  ];
  rect(M, gridTop - cellH * 3, W, cellH * 3, { borderColor: LINE, borderWidth: 0.75 });
  for (const cell of cells) {
    ...
    if (cell.r < 2) line(x, top - cellH, x + w, top - cellH, 0.75, LINE);
  ...
  t('Work days = (SF / production rate) / crew, rounded up. Loads = debris cu yd / usable capacity, rounded up. Workdays run Mon-Fri.', M, y - 10, { size: 6.8, color: MUTED });
...
export async function sendProjectSchedule(input, { fetchImpl = fetch, url = process.env.PROJECT_SCHEDULE_WEBHOOK || PROJECT_SCHEDULE_WEBHOOK } = {}) {
  ...
  const summary = {
    ...
    startDate: input.start, plannedFinish, squareFeet: input.sf,
    dumpsterSize: input.dumpSize, dumpsterUsable: input.usable,
  ...
  form.append('phases', JSON.stringify(schedule.rows.map(r => ({
    ...
    personDays: Math.round(r.personDays * 100) / 100, productionRate: r.rate, debrisYield: r.yld
  }))));
