...
// Night work: the shift starts in the PM and ends the next morning. Stored with the job's
// schedule and sent to Make with every job payload; a reset leaves it alone.
const nightFlag = v => v === true || v === 'true' || v === 1 || v === '1' || v === 'on';
function applyNight(data, input) { if (has(input, 'nightWork')) data.nightWork = nightFlag(input.nightWork); }
...
export function mutateJob(current, input) {
  ...
  switch (action) {
    ...
    case 'update_details': Object.assign(data, validateDetails(pick(input, DETAILS))); applyNight(data, input); break;
    case 'assign': {
    ...
    case 'patch': Object.assign(data, validateDetails(pick(input, DETAILS))); applyNight(data, input); break;
